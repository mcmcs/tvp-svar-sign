...
% _2MPshocks: two shocks specification
...
if ind_sample == 1 %default
    data   = readtable([frequency,'_data/csvfiles_2MPshocks/dataset.csv']);
    % data   = data(find(strcmp(string(data.dates),'1959-01-01')==1):find(strcmp(string(data.dates),'2023-04-01')==1),:);
    data = data( ( data.dates >= datetime('1959-01-01') ) & (data.dates <= datetime('2023-04-01')), :);

elseif ind_sample == 2
    data   = readtable([frequency,'_data/csvfiles_2MPshocks_2024-07-26/data2024-07-26.csv']);
end
...
data  = [100*diff(log(data.GDPC1)) 100*diff(log(data.PCEPILFE)) data.FEDFUNDS(2:end,1)/4 data.T10YFFM(2:end,1)/4 data.BAA10YM(2:end,1)/4];
...
% -------------------------------------------------------------------------
% Restrictions:
type_restriction = 97; % 0 unrestricted, 1 baseline, 2 tyalor
...
switch type_restriction
    ...
    case 91
        if ind_sample ==1
            function_restrictions = @restriction_baseline_2shocks_taylor1s_cs;
        elseif ind_sample ==2
            function_restrictions = @restriction_baseline_2shocks_taylor1s_cs_20240726;    
        end
        is_restrict_B = true; %true if restriction includes B
    
    case 97
        if ind_sample ==1
            function_restrictions = @restriction_baseline_2shocks_taylor1s_cspi;
            is_restrict_B = true;
        end
end
...
% frequency of Q updating
rQ = Inf; %Inf = update all the time
...
if type_restriction == 0
    ...
    %% Posterior mean
    %load(['results',filesep,['temp_results_2s_unrest','_sam',num2str(ind_sample), '_ndraws_1000.mat']], 'mat_B', 'mat_ddelta', 'mat_ggamma', 'mat_Vd', 'mat_Vg', 'mat_VB');
    load(['results',filesep,'temp_results_jointpr_2s_rest97_sam1r991ndraws10000.mat'], 'mat_B', 'mat_ddelta', 'mat_ggamma', 'mat_Vd', 'mat_Vg', 'mat_VB');
...
save_every   = 100;             % save every save_every
...
switch type_restriction
    ...
    case 0
        save(['temp_results_2s_unrest','_sam',num2str(ind_sample), '_ndraws_',num2str(M0),'.mat'],'mat_B','mat_ddelta','mat_ggamma','mat_VB','mat_Vd','mat_Vg','mat_logL');
    otherwise
        ...
        save(['temp_results_jointpr_2s_rest', num2str(type_restriction), '_sam', num2str(ind_sample), 'r' ,num2str(ind_chain),'ndraws',num2str(M0),'.mat']);
