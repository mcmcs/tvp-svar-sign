function [St_old, DCDhalft_old] = restriction_baseline_2shocks_taylor1s_cspi(B_old_t, ddelta_t, ggamma_t, Q_t, t)
% checking sign restrictions

% 2shocks_taylor1s
% - identify two shocks
% - fourth variable = term spread instead of money
% - cs: put restrictions on cs (both shocks), remove restriction on pi (second shock)
...
ttime_ffr1 = [1969.75:0.25:1979.50, ...
    1983.00:0.25:1983.75];

ttime_ffr2 = [1984.00:0.25:2008.75, ...
    2015.75:0.25:2020.00, ...
    2022.00:0.25:2023.25];
...
% ttime_post84 = [1984.00:0.25:2023.25];
...
if cond_ssigma_old<10000
    ...
    sign_indicator_zlb0  = (L0(3,1)>0)*(L0(4,1)<0)*(L0(2,1)<0)*(L0(5,1)>0);
    sign_indicator_zlb1  = (L0(3,1)>0)*(L0(4,1)<0)*(L0(2,1)<0)*(L0(5,1)>0);
    sign_indicator_zlb2  = (L0(3,1)>0)*(L0(4,1)<0)*(L0(2,1)<0)*(L0(5,1)>0);
    ...
    sign_indicator_ffr1 = (At(1,1)<0)*(At(2,1)<0)*(At(3,1)>0)*(At(5,1)>0)*(L0(3,1)>0)*(L0(4,1)<0)*(L0(2,1)<0)*(ppsi_y<4)*(ppsi_p<4)*(ppsi_cs>-4)*(L0(5,1)>0); %cs added
    sign_indicator_ffr2 = (res_to_p(hrz_impose)>0); % 
    sign_indicator_ffr = sign_indicator_ffr1 & sign_indicator_ffr2; %combine both

    sign_indicator_ffr3 = sign_indicator_ffr1 & (res_to_p(hrz_impose)>1); % taylor principle only after 84

    St_ffr_old = ( (sum(ttime(t)==ttime_ffr1)==1 && sign_indicator_ffr==1) ...
        || (sum(ttime(t)==ttime_ffr2)==1 && sign_indicator_ffr3==1) ...
        || (sum(ttime(t)==ttime_zlb0)==1 && sign_indicator_zlb0==1) ...
        || (sum(ttime(t)==ttime_zlb1)==1 && sign_indicator_zlb1==1) ...
        || (sum(ttime(t)==ttime_zlb2)==1 && sign_indicator_zlb2==1)); % first shock and systematic

    % St_ffr_old = ((sum(ttime(t)==ttime_ffr)==1 && sign_indicator_ffr==1) ...
    %     || (sum(ttime(t)==ttime_zlb0)==1 && sign_indicator_zlb0==1) ...
    %     || (sum(ttime(t)==ttime_zlb1)==1 && sign_indicator_zlb1==1) ...
    %     || (sum(ttime(t)==ttime_zlb2)==1 && sign_indicator_zlb2==1)); % first shock and systematic

    %St_ts_old = (L0(3,2)>0) * (L0(4,2)>0) * (L0(2,2)<0); % second shock
    %St_ts_old = (L0(3,2)>0) * (L0(4,2)>0) * (L0(5,2)<0); % second shock (no effect on pi, effect on cs)
    St_ts_old = (L0(3,2)>0) * (L0(4,2)>0) * (L0(2,2)<0) * (L0(5,2)<0); % second shock (bring rest on pi back, effect on cs)

    St_old = (St_ffr_old==1 && St_ts_old==1);
