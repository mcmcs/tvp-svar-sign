function [ddelta_old, ggamma_old, ntry_oldD, ntry_oldC] = draw_DCD_single_move_split(yhat_old, B_old, ddelta_old, ggamma_old, Q_old, Vd_old, Vg_old, ddelta_hat, ddelta_hat_Vinv, ggamma_hat, ggamma_hat_Vinv, function_restrictions)

% _split: split updating into D and C
...
for t=1:T
    ...
    % Restriction function at time t
    ...
    slice.scale_z    = 1;
    slice.mean       = ddelta_pr_m;
    slice.chol_cov_z = chol(ddelta_pr_V,'lower');
    slice.fcn_lik    = @(z_prop) loglike_yt_DCD_onlyD(y_t1, B_t1, Q_t1, function_restrictions_t, ggamma_t1, z_prop);
    slice.nobs       = nv;
    ...
    slice.scale_z    = 1;
    slice.mean       = ggamma_pr_m;
    slice.chol_cov_z = chol(ggamma_pr_V,'lower');
    slice.fcn_lik    = @(z_prop) loglike_yt_DCD_onlyC(y_t1, B_t1, Q_t1, function_restrictions_t, ddelta_t1, z_prop);
    slice.nobs       = ng;
