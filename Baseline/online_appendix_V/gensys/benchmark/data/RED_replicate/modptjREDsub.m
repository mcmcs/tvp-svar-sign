 function [G1,C,impact,eu,SDX,zmat,NY,NX,NETA,gev,ncof]=modptjREDsub(estpr,calpr,indp,con,ind_sv)
% =========================================================================
% MOD PTJ RED sub
% 
% Single sample estimation of baseline mode RED 2010 
...
%    equation
% The solution for each subsample takes the form:  x(t) = G* x(t-1) + R* e(t)
%                               y(t) = zmat * x(t) + C
%                               e(t) ~ N(0,SDX'*SDX)  
%
% =========================================================================
...
% -----------------------
numpar=      41;  % Number of parameters 
ncof=        33;  % Number of coefficients not corresponding to 
                  % standard deviations 
...
[G1,C,impact,~,~,~,gev,eu]=gensys(GAM0,GAM1,C,PSI,PPI) ;

if flag_relpiobs==1
    zmat=zeros(8,size(G1,2));
else
