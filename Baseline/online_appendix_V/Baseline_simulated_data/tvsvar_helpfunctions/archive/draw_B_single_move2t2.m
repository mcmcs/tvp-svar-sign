function [B_old, ntry_old] = draw_B_single_move2t2(y_old, x_old, B_old, ddelta_old, ggamma_old, Q_old, VB_old, bbet_hat, bbet_hat_Vinv, function_restrictions)
...
for t=1:T
    ...
    elseif t == 1
        VB_inv = eye(k) / VB_old;
        bbet_pr_V = eye(k) / ( VB_inv + bbet_hat_Vinv);
        bbet_pr_m = bbet_pr_V * (VB_inv*bbet_t2 + bbet_hat_Vinv*bbet_t0);
    end
%         bbet_pr_V = eye(k) / ( bbet_hat_Vinv);
%     elseif t == 1
%         VB_inv = eye(k) / VB_old;
