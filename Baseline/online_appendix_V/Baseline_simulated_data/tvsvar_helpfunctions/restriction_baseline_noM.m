function [St_old, DCDhalft_old] = restriction_baseline_noM(B_old_t, ddelta_t, ggamma_t, Q_t, t)
...
% --- Actual checks
sign_indicator_ffr = (At(1,1)<0)*(At(2,1)<0)*(At(3,1)>0)*(At(5,1)>0)*(L0(3,1)>0)*(L0(4,1)<0)*(L0(2,1)<0)*(ppsi_y<4)*(ppsi_p<4)*(ppsi_cs>-4);
...
%     || (sum(ttime(t)==ttime_zlb0)==1) ...
%     || (sum(ttime(t)==ttime_zlb1)==1) ...
%     || (sum(ttime(t)==ttime_zlb2)==1));
